...
#                                           certificate
#
# The .command files are thin entries that point at start.sh / reset.sh next
# to THIS script, wherever the repo lives.
#
...
#
# With --start-command-only it writes only start.command and
...
if [ ! -x "$START_SH" ] || [ ! -x "$RESET_SH" ]; then
  echo "ERROR: expected start.sh and reset.sh next to this script — run this" >&2
  echo "from the repo." >&2
  exit 1
fi
if [ ! -d "$DESKTOP" ]; then
