...
#                                           points at start.sh next to THIS
#                                           script, wherever the repo lives
#
...
#
# With --start-command-only it writes just start.command — for setups that
# don't use the self-signed certificate (e.g. Tailscale), where there is no CA
# to hand over and nothing to install on the phone.
#
...
if [ ! -x "$START_SH" ]; then
  echo "ERROR: expected $START_SH next to this script — run this from the repo." >&2
  exit 1
fi
if [ ! -d "$DESKTOP" ]; then
...
# start.command: Finder runs an executable .command file in a new Terminal
# window. Terminal starts it in $HOME, so bake in the absolute path of the
# start.sh that lives next to this script (plus any --start-args).
